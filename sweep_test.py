...
def create_haystacks(n, dSNRs, NeedleType, NeedleSize, path ):
    dSNR_levels = dSNRs
    #Create haystacks with needles
    for dSNR in dSNR_levels:
        run(['python', 'rand_hay.py', str(n), str(NeedleType), '--dSNR', str(dSNR), '--NeedleSize', str(NeedleSize), '--out_path', path])
    #Create empty haystacks
    run(['python', 'rand_hay.py', str(n), 'noise', '--out_path', path])
    
def sweep_sizes(etype, path):
    sweeps = [2**10, 2**11, 2**12, 2**13, 2**15, 2**16, 2**17, 2**18, 2**19, 1000000]
    for window in sweeps:
        run(['python', 'scan.py', etype, path, '--sweep', str(window)])
    print('Ran scan on on exponentially decreasing window sizes. Check out results in ' + path)

def plot_mc(etype, dSNRs, path):
    results = pd.read_csv(path + etype + '_results.csv', index_col=0)
    ...
    plt.savefig(path + etype + '_' + '_plot.png')
    #plt.show()

    print('Completed plotting. Check out plots in ' + path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('path', type=str, default="sweep_test/", help= 'specify path to save all files and plots')
    ...
    parser.add_argument('--NeedleType', choices=['sine', 'chirp', 'BPSK', 'BFSK', 'noise', 'normnoise'], default='sine', help='injected signal type {sine, chirp, BPSK, BFSK, noise}')
    ...
    create_haystacks(args.n, args.dSNRs, args.NeedleType, args.NeedleSize, args.path)
    sweep_sizes(args.etype, args.path)
    plot_mc(args.etype, args.dSNRs, args.path)
